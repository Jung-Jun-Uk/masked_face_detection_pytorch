...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
            ...
            for line in lines:
                ...
                if line.startswith('#'):
                    if isFirst is True:
                        ...
                        labels.clear()
                    p = line[2:]
                    p = path.replace('label.txt','images/') + p
        ...
        # Check cache
        cache_path = 'data/widerface' + '.cache'  # cached labels
    ...
    def cache_labels(self, path='labels.cache'):
        data_dict = {}     
        pbar = tqdm(zip(self.img_files, self.words), desc='Scanning images', total=len(self.img_files))       
        for img, labels in pbar:                    
            img0 = cv2.imread(img) # BGR
            height, width, _ = img0.shape            
            shape = (width, height)
            gn = torch.tensor(img0.shape)[[1, 0, 1, 0]]  # normalization gain whwh            
            xyxy_bboxes = []
            for l in labels:      
                clsxyxy = torch.zeros(1,5)      
                xyxy = torch.Tensor([l[0], l[1], l[0] + l[2], l[1] + l[3]]).view(1,4) # x1y1x2y2                
                xyxy = torch.clamp(xyxy, min=torch.ones_like(xyxy), max=(gn-1).unsqueeze(0))                 
                xywh = xyxy2xywh(xyxy / gn)
                clsxyxy[0,1:] = xywh                        
                xyxy_bboxes.append(clsxyxy)
            xyxy_bboxes = torch.cat(xyxy_bboxes, dim=0).numpy()                                
            if len(xyxy_bboxes) == 0:
                xyxy_bboxes = np.zeros((0,5), dtype=np.float32)
            data_dict[img] = [xyxy_bboxes, shape]
