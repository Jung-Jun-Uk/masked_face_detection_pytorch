...
#from utils.datasets_temp import create_dataloader
import test_widerface 
...
def train(hyp, opt, device, tb_writer=None):
    ...
    dir_path, path = data_dict['train_dir'], data_dict['train']
    ...
    # Trainloader
    dataloader, dataset = create_dataloader(data_dict['train_dir'], data_dict['train'], imgsz, batch_size, gs, opt, hyp=hyp, augment=True,
    ...
    if rank in [-1, 0]:
        ...
        testloader = create_dataloader(data_dict['val_dir'], data_dict['val'], imgsz_test, batch_size, gs, opt, hyp=hyp, augment=False,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            if not opt.notest or final_epoch:  # Calculate mAP
                results, maps, times = test_widerface.test(opt.data,
...
def parser():
    ...
    parser.add_argument('--weights', type=str, default='runs/exp3_yolov4-p5-896-tune/weights/last_yolov4-p5-896-tune.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default='yolov4-p5.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='data/pwmfd.yaml', help='data.yaml path')
